...
TEST(Matrix, OperatorCircleBraces)
{
	...
	matrix::Matrix<int32_t> matrix(mtx);
...
TEST(Matrix, OperatorEqual)
{
	...
	const std::vector<std::vector<int32_t>> mtx_2 = mtx_1;

	const matrix::Matrix<int32_t> matrix_1(mtx_1);
	const matrix::Matrix<int32_t> matrix_2(mtx_2);
...
TEST(Matrix, OperatorAddAndAssign)
{
	...
	matrix::Matrix<int32_t> matrix_1(mtx_1);
	const matrix::Matrix<int32_t> matrix_2(mtx_2);
...
TEST(Matrix, MultiplicationMatrixByNum)
{
	...
	const matrix::Matrix<int32_t> sourceMatrix(mtx);
...
{
	const std::vector<std::vector<uint32_t>> mtx_1 = {
		{ 24, 65, 46, 21, 11 },
		{ 12, 96, 25, 14, 12 },
		{ 11, 32, 16, 96, 13 },
		{ 81, 13, 18, 25, 14 },
	...
	const std::vector<std::vector<uint32_t>> positiveShiftedMatrix = {
		{ 11, 12, 24, 65, 46 },
		{ 81, 13, 32, 96, 21 },
		{ 84, 18, 16, 25, 11 },
		{ 13, 25, 96, 14, 12 },
	...
	matrix::Matrix<uint32_t> matrix(mtx_1);
	const int8_t positiveOffset = 2;
	const int8_t negativeOffset = -2;

	matrix::Matrix<uint32_t> resulMatrix = matrix::ShiftMatrix(matrix, positiveOffset);
	const matrix::Matrix<uint32_t> validMatrixWithPositiveShift(positiveShiftedMatrix);
